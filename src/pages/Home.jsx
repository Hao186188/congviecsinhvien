...
import './Home.css';
// import apiService from '../services/api'; // Comment hoặc xóa dòng này
import { jobService } from '../services/jobService'; // Thêm import mới
...
function Home() {
  ...
  const handleCategoryClick = (categoryValue) => {
    navigate(`/jobs?category=${categoryValue}`);
  };
  ...
  const loadFeaturedJobs = async () => {
    try {
      // Sử dụng service mới thay vì API
      const response = await jobService.getFeaturedJobs();
      setFeaturedJobs(response.data.jobs || []);
      console.log(`Loaded ${response.data.jobs.length} featured jobs from sample data`);
    } catch (error) {
      console.error('Error loading featured jobs:', error);
      // Fallback: lấy 6 công việc đầu tiên từ dữ liệu mẫu
      const { getAllSampleJobs } = require('../data/sampleJobs');
  ...
  // Xóa mảng categories cứng và sử dụng JOB_CATEGORIES từ data/kienGiang
        ...
        <div className="container">
          <div className="hero-content">
            ...
            <form className="search-box" onSubmit={handleSearch}>
              ...
              />
              {/* Sửa: Thay thế select cứng bằng allLocations */}
              <select 
                value={location}
      ...
      <section className="categories">
        <div className="container">
          ...
          <div className="category-grid">
            {/* Sửa: Sử dụng JOB_CATEGORIES và thêm onClick handler */}
            {categories.map((category, index) => (
              <div 
                key={category.value} 
                className="category-item"
                // Thêm sự kiện click để liên kết
                onClick={() => handleCategoryClick(category.value)}
                style={{ cursor: 'pointer' }} // Gợi ý CSS
              >
